...
RSpec.describe Counters::DumpService do
  ...
  context 'when information presents' do
    before do
      redis.call('SET', "counters:photo:#{photo_with_views.id * 2}", 1_000)
      redis.call('SET', "counters:photo:#{photo_without_views.id}", 100)
      redis.call('SET', "counters:photo:#{photo_with_views.id}", 10)
    end
    ...
    context 'when regular call' do
      it do
        ...
        expect(redis.call('KEYS', '*')).to contain_exactly(
          "counters:photo:#{photo_with_views.id * 2}",
          "counters:photo:#{photo_without_views.id}",
          "counters:photo:#{photo_with_views.id}"
        )

        expect(redis.call('TTL', "counters:photo:#{photo_with_views.id * 2}")).to be_positive

        expect(redis.call('GET', "counters:photo:#{photo_without_views.id}")).to eq('0')
        expect(redis.call('TTL', "counters:photo:#{photo_without_views.id}")).to be_positive

        expect(redis.call('GET', "counters:photo:#{photo_with_views.id}")).to eq('0')
        expect(redis.call('TTL', "counters:photo:#{photo_with_views.id}")).to be_positive
      end
    end
  end
end
