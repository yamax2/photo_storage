...
RSpec.describe Yandex::Token do
  describe 'structure' do
    ...
    it { is_expected.to have_db_column(:refresh_token).of_type(:string).with_options(null: false, limit: 255) }
    ...
    it { is_expected.to have_db_column(:folder_indexes).of_type(:jsonb).with_options(null: false) }
  end
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_length_of(:refresh_token).is_at_most(255) }
    ...
    %i[photos_folder_index other_folder_index photos_folder_archive_from other_folder_archive_from].each do |attr|
      it { is_expected.to validate_presence_of(attr) }
      it { is_expected.to validate_numericality_of(attr).is_greater_than_or_equal_to(0).only_integer }
    end
  end
